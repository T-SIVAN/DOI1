...
一个本地 Streamlit 网页工具，面向生物医学科研场景，包含 PDF 精读、四源文献检索、引用追踪、模块化科研写作、科研绘图和 PPT 汇报。
...
- `科研写作`：拆分为写作与润色、审稿与回复、引用与数据、科研绘图、成果转化和能力覆盖六个工作区，避免把不同输入混在一个长表单中。
...
- 网页内“能力覆盖”会按 `完整 / 部分 / 未实现` 核对上游 `nature-skills`。通用 LLM 提示词只标记为“部分”，不会宣称已实现上游完整工作流。
