...
一个本地 Streamlit 网页工具，面向生物医学科研场景，包含 PDF 精读、四源文献检索、引用追踪、模块化科研任务、科研绘图、PPT 汇报和连续对话。
...
- `科研任务`：覆盖写作与润色、基金策划、审稿与回复、引用与数据、参考文献核验、统计方案、实验日志和成果转化。每个文本任务都支持 PDF、Word、PPT、表格、Markdown、RIS 与 BibTeX 附件。
- `科研绘图`：上传 CSV/XLSX，生成散点、折线、柱状、箱线、小提琴或相关性热图；支持 300/600 DPI PNG、可编辑 SVG 和 PDF 导出，不需要 LLM Key。
...
- `科研对话`：保留当前会话上下文，可连续追问，并在任意一轮加入论文、表格或文稿附件。

主导航位于侧栏，移动端不再依赖容易横向截断的多层标签栏。原“能力覆盖”核对页已移除，功能边界与本次实现情况只在开发交付说明中汇报。
...
- 附件会在当前 Streamlit 会话中读取并按模型上下文长度截断；扫描版 PDF 需要先 OCR，网页不会把附件写入持久化数据库。
- 科研绘图首版采用 Python/matplotlib。它负责真实图表渲染和导出；R、多面板自动排版、统计检验与功效分析仍属于后续扩展。
- 绘图库采用按需加载：即使云端绘图依赖暂时不可用，PDF 精读、文献检索、引用追踪和写作功能仍可正常启动，并在绘图时显示具体错误。
